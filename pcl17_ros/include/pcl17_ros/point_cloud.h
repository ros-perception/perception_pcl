#ifndef PCL17_ROS_POINT_CLOUD_H_
#define PCL17_ROS_POINT_CLOUD_H_

#include <ros/ros.h>
#include <pcl17/point_cloud.h>
#include <pcl17/point_traits.h>
#include <pcl17/for_each_type.h>
...
namespace ros 
{
  ...
  template<typename T>
  struct DefaultMessageCreator<pcl17::PointCloud<T> >
  {
    boost::shared_ptr<pcl17::MsgFieldMap> mapping_;

    DefaultMessageCreator()
      : mapping_( boost::make_shared<pcl17::MsgFieldMap>() )
    {
    }
    
    boost::shared_ptr<pcl17::PointCloud<T> > operator() ()
    {
      boost::shared_ptr<pcl17::PointCloud<T> > msg (new pcl17::PointCloud<T> ());
      pcl17::detail::getMapping(*msg) = mapping_;
      return msg;
    }
  };
#endif
  ...
  {
    template<typename T> struct MD5Sum<pcl17::PointCloud<T> >
    {
      static const char* value() { return MD5Sum<sensor_msgs::PointCloud2>::value(); }
      static const char* value(const pcl17::PointCloud<T>&) { return value(); }
    ...
    template<typename T> struct DataType<pcl17::PointCloud<T> >
    {
      static const char* value() { return DataType<sensor_msgs::PointCloud2>::value(); }
      static const char* value(const pcl17::PointCloud<T>&) { return value(); }
    };

    template<typename T> struct Definition<pcl17::PointCloud<T> >
    {
      static const char* value() { return Definition<sensor_msgs::PointCloud2>::value(); }
      static const char* value(const pcl17::PointCloud<T>&) { return value(); }
    };

    template<typename T> struct HasHeader<pcl17::PointCloud<T> > : TrueType {};
  } // namespace ros::message_traits

  namespace serialization 
  {
    template<typename T>
    struct Serializer<pcl17::PointCloud<T> >
    {
      template<typename Stream>
      inline static void write(Stream& stream, const pcl17::PointCloud<T>& m)
      {
        ...
        typedef typename pcl17::traits::fieldList<T>::type FieldList;
        uint32_t fields_size = boost::mpl::size<FieldList>::value;
        stream.next(fields_size);
        pcl17::for_each_type<FieldList>(pcl17::detail::FieldStreamer<Stream, T>(stream));
      ...
      template<typename Stream>
      inline static void read(Stream& stream, pcl17::PointCloud<T>& m)
      {
        stream.next(m.header);
        ...
        boost::shared_ptr<pcl17::MsgFieldMap>& mapping_ptr = pcl17::detail::getMapping(m);
        if (!mapping_ptr)
        {
          // This normally should get allocated by DefaultMessageCreator, but just in case
          mapping_ptr = boost::make_shared<pcl17::MsgFieldMap>();
        }
        pcl17::MsgFieldMap& mapping = *mapping_ptr;
        if (mapping.empty())
          pcl17::createMapping<T> (fields, mapping);
        ...
        else
        {
          ...
          for (uint32_t row = 0; row < m.height; ++row) {
            ...
            for (uint32_t col = 0; col < m.width; ++col, stream_data += point_step) {
              BOOST_FOREACH(const pcl17::detail::FieldMapping& fm, mapping) {
      ...
      inline static uint32_t serializedLength(const pcl17::PointCloud<T>& m)
      {
        uint32_t length = 0;
        ...
        pcl17::detail::FieldsLength<T> fl;
        typedef typename pcl17::traits::fieldList<T>::type FieldList;
        pcl17::for_each_type<FieldList>(boost::ref(fl));
