#ifndef PCL16_ROS_POINT_CLOUD_H_
#define PCL16_ROS_POINT_CLOUD_H_

#include <ros/ros.h>
#include <pcl16/point_cloud.h>
#include <pcl16/point_traits.h>
#include <pcl16/for_each_type.h>
...
namespace ros 
{
  ...
  template<typename T>
  struct DefaultMessageCreator<pcl16::PointCloud<T> >
  {
    boost::shared_ptr<pcl16::MsgFieldMap> mapping_;

    DefaultMessageCreator()
      : mapping_( boost::make_shared<pcl16::MsgFieldMap>() )
    {
    }
    
    boost::shared_ptr<pcl16::PointCloud<T> > operator() ()
    {
      boost::shared_ptr<pcl16::PointCloud<T> > msg (new pcl16::PointCloud<T> ());
      pcl16::detail::getMapping(*msg) = mapping_;
      return msg;
    }
  };
#endif
  ...
  {
    template<typename T> struct MD5Sum<pcl16::PointCloud<T> >
    {
      static const char* value() { return MD5Sum<sensor_msgs::PointCloud2>::value(); }
      static const char* value(const pcl16::PointCloud<T>&) { return value(); }
    ...
    template<typename T> struct DataType<pcl16::PointCloud<T> >
    {
      static const char* value() { return DataType<sensor_msgs::PointCloud2>::value(); }
      static const char* value(const pcl16::PointCloud<T>&) { return value(); }
    };

    template<typename T> struct Definition<pcl16::PointCloud<T> >
    {
      static const char* value() { return Definition<sensor_msgs::PointCloud2>::value(); }
      static const char* value(const pcl16::PointCloud<T>&) { return value(); }
    };

    template<typename T> struct HasHeader<pcl16::PointCloud<T> > : TrueType {};
  } // namespace ros::message_traits

  namespace serialization 
  {
    template<typename T>
    struct Serializer<pcl16::PointCloud<T> >
    {
      template<typename Stream>
      inline static void write(Stream& stream, const pcl16::PointCloud<T>& m)
      {
        ...
        typedef typename pcl16::traits::fieldList<T>::type FieldList;
        uint32_t fields_size = boost::mpl::size<FieldList>::value;
        stream.next(fields_size);
        pcl16::for_each_type<FieldList>(pcl16::detail::FieldStreamer<Stream, T>(stream));
      ...
      template<typename Stream>
      inline static void read(Stream& stream, pcl16::PointCloud<T>& m)
      {
        stream.next(m.header);
        ...
        boost::shared_ptr<pcl16::MsgFieldMap>& mapping_ptr = pcl16::detail::getMapping(m);
        if (!mapping_ptr)
        {
          // This normally should get allocated by DefaultMessageCreator, but just in case
          mapping_ptr = boost::make_shared<pcl16::MsgFieldMap>();
        }
        pcl16::MsgFieldMap& mapping = *mapping_ptr;
        if (mapping.empty())
          pcl16::createMapping<T> (fields, mapping);
        ...
        else
        {
          ...
          for (uint32_t row = 0; row < m.height; ++row) {
            ...
            for (uint32_t col = 0; col < m.width; ++col, stream_data += point_step) {
              BOOST_FOREACH(const pcl16::detail::FieldMapping& fm, mapping) {
      ...
      inline static uint32_t serializedLength(const pcl16::PointCloud<T>& m)
      {
        uint32_t length = 0;
        ...
        pcl16::detail::FieldsLength<T> fl;
        typedef typename pcl16::traits::fieldList<T>::type FieldList;
        pcl16::for_each_type<FieldList>(boost::ref(fl));
